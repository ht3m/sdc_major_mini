...
if __name__ == "__main__":
    ...
    try:
        ...
        # 2. 合并节点 (使用延迟删除，避免断链)
        builder.merge_close_nodes(distance_threshold=20.0) 
        
        # 3. 兜底扫描 (挽救合并或追踪中遗失的路径)
        builder.salvage_missing_segments(min_length=20)

        # 4. 美容 (修剪毛刺 & 去噪) ---
        builder.prune_spurs(min_length=10) # 小于20像素的短枝剪掉
        builder.remove_small_components(min_nodes=10, min_total_length=10) # 极小的孤立噪点删掉
